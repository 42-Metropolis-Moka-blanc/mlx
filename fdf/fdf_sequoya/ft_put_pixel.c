...
/*   Updated: 2015/02/04 06:50:37 by rbaum            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		fdf_draw_line_t2(t_env *e)
{
	...
	if (abs(dx) > abs(dy))
		while (dx += (dx < 0) - (dx > 0))
			mlx_pixel_put(e->mlx, e->win, e->xa + dx,e->ya + dx * (e->yc - e->ya) / (e->xc - e->xa), color);
	else
		while (dy += (dy < 0) - (dy > 0))
			mlx_pixel_put(e->mlx, e->win, e->xa + dy * (e->xc - e->xa) / (e->yc - e->ya),
						  e->ya + dy, color);
	return (0);
}
